...
export function GogAccountsCard({ defaultServices }: Props = {}) {
  ...
  const refresh = useCallback(async () => {
    try {
      const [a, s, f] = await Promise.all([
        api.gogAccounts(), api.gogServices(), api.gogFlowStatus(),
      ]);
      setAccounts(a); setServices(s); setFlow(f);
    } catch (e) {
  ...
  useEffect(() => {
    if (flow.status === "success") {
      ...
      api.gogResetAuth().then(setFlow).catch(() => {});
  ...
  async function cancelAuth() {
    try { setFlow(await api.gogCancelAuth()); } catch {}
  }
      ...
      {flow.status === "running" && (
        <div className="mb-3 flex items-start gap-2 p-3 rounded-md
          ...
          <div className="flex-1 min-w-0">
            ...
            {flow.auth_url && (
              <a href={flow.auth_url} target="_blank" rel="noopener noreferrer"
...
/* ── ACCOUNT ROW ──────────────────────────────────────────────────── */
function AccountRow({ account, allServices, onAddPerms, disabled }: {
  account:     GogAccount;
    ...
    <Surface level={2} className="p-3 bg-bg/40">
      ...
      <div className="flex flex-wrap gap-1">
        {account.services.length === 0 ? (
          ...
          account.services.sort().map((s) => (
            <ServiceChip key={s} service={s} authorized />
          ))
        )}
...
/* ── MODAL DE AUTH ────────────────────────────────────────────────── */
function AuthModal({ prefillAccount, prefillServices, allServices, onCancel, onConfirm }: {
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-[12px] text-text-dim mt-0.5">
              Al confirmar, se abrirá el navegador para que des consentimiento.
              Marcá <strong>todos</strong> los checkboxes en la pantalla de Google.
            </p>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {allServices.length === 0 && (
              <span className="text-xs text-text-dim italic col-span-2">
                Cargando catálogo…
              </span>
            )}
            {allServices.map((svc) => {
