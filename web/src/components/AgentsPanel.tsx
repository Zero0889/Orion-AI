...
const ICON_TONES: Record<string, string> = {
  compass:  "text-amber-400",  search:   "text-sky-400",
  code:     "text-emerald-400", sigma:   "text-violet-400",
  feather:  "text-rose-400",   chart:    "text-orange-400",
  folder:   "text-cyan-400",   sensors:  "text-lime-400",
  sparkles: "text-pink-400",   orbit:    "text-pri",
  cpu:      "text-blue-400",   memory:   "text-fuchsia-400",
  chat:     "text-teal-400",   bolt:     "text-yellow-400",
};
...
export function AgentsPanel() {
  ...
  const [sessions, setSessions] = useState<AgentSessions>(
    () => loadSessions(),
  );
  ...
  const currentSession = (currentSessionId && activeAgentId)
    ? ((sessions[activeAgentId] ?? []) as ChatSession[]).find((s: ChatSession) => s.id === currentSessionId) ?? null
    : null;
    ...
    <div className="flex flex-col h-full">
      <SectionHeader
        ...
        action={activeAgent ? (
          ...
          <Button size="sm" variant="primary" icon="add" onClick={() => {
            const first = providers[0];
            setNewDraft({
              id: "", role: "", icon: "sparkles", description: "",
              provider: first?.id ?? "gemini", model: first?.models?.[0]?.id ?? "",
              temperature: 0.7, tools: ["*"], system: "",
              enabled: true, fallback_provider: null, fallback_model: null, available: true,
            });
            setCreatingNew(true);
          }}>
            Crear agente
      ...
      {error && (
        <div className="mx-6 mt-3 flex items-start gap-2 p-3 rounded-md border border-danger/30 bg-danger/10 text-xs text-danger">
          ...
          <span>{error}</span>
          <button className="ml-auto text-danger/70 hover:text-danger" onClick={() => setError(null)}>
      ...
      <div className="flex-1 overflow-hidden">
        {loading ? (
          ...
          <Empty
            ...
            action={<Button variant="primary" size="sm" icon="add" onClick={() => {
              setNewDraft({
                id: "", role: "", icon: "sparkles", description: "",
      ...
      <AgentEditorModal
        ...
        providers={providers}
        onClose={() => { setEditing(null); setCreatingNew(false); setNewDraft(null); }}
        onSaved={() => { setEditing(null); setCreatingNew(false); setNewDraft(null); }}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-3">
        <span className={`grid place-items-center h-10 w-10 rounded-xl bg-white/[0.04] ${tone}
...
function AgentChatView({
  agent, sessions, activeSessionId, messages, onNewMsg,
  ...
  return (
    <div className="flex h-full">
      {/* Session sidebar */}
      <div className={`${sidebarOpen ? "w-56" : "w-0"} border-r border-white/[0.06] bg-sunken/40
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin px-6 py-4 space-y-4">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center gap-3 px-8">
              <span className={`grid place-items-center h-14 w-14 rounded-2xl bg-pri/10 ${tone} mb-2`}>
                <Icon name={(agent.icon as IconName) || "chat"} size={24} />
              </span>
              <h3 className="text-lg font-semibold text-text">{agent.role}</h3>
              {agent.description && (
                <p className="text-sm text-text-dim max-w-sm leading-relaxed">{agent.description}</p>
              )}
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : ""}`}>
              {msg.role === "agent" && (
                <span className={`shrink-0 h-7 w-7 grid place-items-center rounded-lg bg-white/[0.04] ${tone}`}>
            ...
            <div className="flex gap-3">
              ...
              <div className="rounded-xl px-4 py-2.5 bg-elevated border border-white/[0.06]">
                <span className="flex items-center gap-1.5 text-xs text-text-dim">
                  <span className="h-1 w-1 rounded-full bg-pri animate-bounce" style={{ animationDelay: "0ms" }} />
                  <span className="h-1 w-1 rounded-full bg-pri animate-bounce" style={{ animationDelay: "150ms" }} />
                  <span className="h-1 w-1 rounded-full bg-pri animate-bounce" style={{ animationDelay: "300ms" }} />
                </span>
              </div>
            </div>
          )}
...
function AgentEditorModal({
  agent, isNew, newDraft, providers, onClose, onSaved, onError,
}: {
  ...
  const ICONS: { id: string; label: string }[] = [
    { id: "sparkles", label: "Sparkles" }, { id: "search",   label: "Buscar" },
    { id: "code",     label: "Código" },    { id: "sigma",    label: "Sigma" },
    { id: "feather",  label: "Pluma" },     { id: "chart",    label: "Gráfico" },
    { id: "folder",   label: "Carpeta" },   { id: "sensors",  label: "Sensor" },
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      const spec: AgentSpec = { id, role, icon, description, provider, model, temperature,
        tools: tools.split(",").map((t) => t.trim()).filter(Boolean),
        system, enabled, fallback_provider: fallbackProvider || undefined,
        fallback_model: fallbackModel || undefined,
      };
      if (creating) {
        await api.createAgent(spec);
      } else {
        const patch = { ...spec }; delete patch.id;
          ...
          <div>
            ...
            {deleteConfirm && (
              <span className="flex items-center gap-2">
                ...
                <Button variant="danger" size="sm" onClick={remove} loading={busy}>Sí</Button>
                <Button variant="ghost" size="sm" onClick={() => setDelConf(false)}>No</Button>
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="text-[10px] uppercase tracking-[0.18em] text-muted">ID (snake_case)</span>
            <input className={inputCls} value={id} onChange={(e) => setId(e.target.value)}
                   placeholder="mi_agente" disabled={!creating} />
          </label>
          <label className="block">
        ...
        <label className="block">
          ...
          <input className={inputCls} value={description} onChange={(e) => setDesc(e.target.value)}
                 placeholder="Qué hace este agente…" />
        </label>

        {/* Icon picker */}
        <div>
          <span className="text-[10px] uppercase tracking-[0.18em] text-muted block mb-2">Icono</span>
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            ...
            <select className={inputCls} value={provider} onChange={(e) => { setProvider(e.target.value); setModel(""); }}>
              {providers.map((p) => (
          ...
          <label className="block">
            ...
            <input className={inputCls} value={model} onChange={(e) => setModel(e.target.value)}
                   list="suggested-models" placeholder="gemini-2.5-flash" />
        ...
        <label className="block">
          ...
          <input
            ...
            value={temperature} onChange={(e) => setTemp(parseFloat(e.target.value))}
        ...
        {/* Tools */}
        <label className="block">
          <span className="text-[10px] uppercase tracking-[0.18em] text-muted">Tools (* = todas)</span>
        ...
        <label className="block">
          ...
          <textarea className={`${inputCls} min-h-[80px] resize-y`} value={system}
                    onChange={(e) => setSystem(e.target.value)} rows={3}
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-text-dim">Si está deshabilitado, no aparece en la orquesta</p>
          </div>
        ...
        <div className="border-t border-white/[0.06] pt-4">
          ...
          {showFallback && (
            <div className="grid grid-cols-2 gap-3 mt-3">
              <label className="block">
                <span className="text-[10px] uppercase tracking-[0.18em] text-muted">Proveedor alternativo</span>
                <input className={inputCls} value={fallbackProvider} onChange={(e) => setFbP(e.target.value)}
                       placeholder="openrouter" />
              </label>
              <label className="block">
                <span className="text-[10px] uppercase tracking-[0.18em] text-muted">Modelo alternativo</span>
                <input className={inputCls} value={fallbackModel} onChange={(e) => setFbM(e.target.value)}
...
function useProviderLabel(provider: string): string {
  const labels: Record<string, string> = {
    gemini: "Gemini", openrouter: "OpenRouter", groq: "Groq",
