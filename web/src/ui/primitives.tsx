  ...
  return (
    <div
      ...
      className={cn(
        "rounded-lg",
        glass    ? "surface-glass"
                 : level === 2 ? "surface-2" : "surface-1",
        hover    && "transition-colors duration-200 ease-out-expo hover:bg-elevated/80 hover:border-white/10",
        className,
      )}
    />
  );
...
const VARIANT: Record<Variant, string> = {
  primary:
    "bg-pri text-bg shadow-glow-soft hover:brightness-110 active:brightness-95 " +
    "disabled:bg-elevated disabled:text-muted disabled:shadow-none",
  secondary:
    "bg-elevated text-text border border-white/[0.06] " +
    "hover:bg-elevated hover:border-white/[0.12] hover:text-text " +
    ...
    <div className={cn(
      ...
      {icon && (
        <div className="relative mb-5">
          ...
          <div className="relative flex h-12 w-12 items-center justify-center rounded-2xl border border-white/[0.08] bg-elevated text-pri">
...
export function SectionHeader({
  ...
  return (
    <header className="flex items-end justify-between px-6 py-5 border-b border-white/[0.06]">
      <div>
        {eyebrow && (
          <div className="text-micro uppercase tracking-[0.24em] text-pri/80 mb-1">{eyebrow}</div>
        )}
...
export function Modal({
  open, onClose, title, eyebrow, footer, size = "md", children,
}: {
  open:    boolean;
  onClose: () => void;
  title:   string;
      ...
      >
        <header className="flex items-start justify-between gap-4 px-6 pt-5 pb-4 border-b border-white/[0.06]">
          <div className="min-w-0">
            {eyebrow && <div className="text-[10px] uppercase tracking-[0.24em] text-pri/80 mb-0.5">{eyebrow}</div>}
...
export function Field({
  ...
  return (
    <label className="block">
      ...
      {children}
      {error && <p className="mt-1.5 text-xs text-danger flex items-center gap-1.5"><Icon name="alert" size={12} /> {error}</p>}
    </label>
  );
}
...
export function Kbd({ children }: { children: ReactNode }) {
  return (
    <kbd className="inline-flex h-5 min-w-5 items-center justify-center px-1.5 rounded
