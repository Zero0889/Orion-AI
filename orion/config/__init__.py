...
def get_base_dir() -> Path:
    """User-writable root (config, data). Next to exe in frozen mode.

    Post Fase 2: el archivo vive en ``orion/config/__init__.py``, así que
    necesitamos tres ``.parent`` para escapar `orion/config/` y llegar al
    project root (antes eran dos cuando vivía en ``config/__init__.py``).
    """
