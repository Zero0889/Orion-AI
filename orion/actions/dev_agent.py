...
from orion.config import PROJECTS_DIR
...
def _run_project(run_command: str, project_dir: Path, timeout: int = 30) -> str:
    ...
    try:
        ...
        return "\n\n".join(combined_parts) if combined_parts else "Ejecutado sin salida."

    except subprocess.TimeoutExpired:
        return f"Tiempo agotado después de {timeout}s — la aplicación de larga duración (servidor/GUI) probablemente está funcionando."
...
def _build_project(
    ...
    for file_info in sorted_files:
        ...
        for attempt in range(2):
            try:
                ...
                if attempt == 0:
                    log("Límite de solicitudes — esperando 20s...")
                    time.sleep(20)
                else:
                    log(f"Reintento de límite falló para {file_path}, omitiendo.")
            except Exception as e:
                log(f"No se pudo escribir {file_path}: {e}")
                break
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        log(f"Vista previa de salida: {last_output[:150]}")

        if not _has_error(last_output, run_command):
            msg = (
                f"El proyecto '{proj_name}' está funcionando, señor. "
        ...
        if error_type == "dependency_error" and auto_installs < 3:
            ...
            if installed:
                auto_installs += 1
                log("Dependencia faltante instalada, reintentando...")
                time.sleep(1)
                continue
