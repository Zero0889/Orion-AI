...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        try:
            ...
            print(f"[Settings] pycaw falló, usando respaldo de teclas: {e}")
            pyautogui.press("volumemute")
            pyautogui.press("volumemute")
    elif _OS == "Darwin":
        subprocess.run(
            ["osascript", "-e", f"set volume output volume {value}"], capture_output=True
        )
        return
    else:
        subprocess.run(
            ["pactl", "set-sink-volume", "@DEFAULT_SINK@", f"{value}%"], capture_output=True
        )
        return
...
def brightness_up():
    if _OS == "Darwin":
        ...
        try:
            ...
            print(f"[Settings] Subir brillo falló en Windows: {e}")
...
def brightness_down():
    if _OS == "Darwin":
        ...
        try:
            ...
            print(f"[Settings] Bajar brillo falló en Windows: {e}")
...
def snap_left():
    if _OS == "Windows":
        ...
        with contextlib.suppress(Exception):
            subprocess.run(
                ["wmctrl", "-r", ":ACTIVE:", "-e", "0,0,0,960,1080"], capture_output=True
            )
...
def snap_right():
    if _OS == "Windows":
        ...
        with contextlib.suppress(Exception):
            subprocess.run(
                ["wmctrl", "-r", ":ACTIVE:", "-e", "0,960,0,960,1080"], capture_output=True
            )
...
def sleep_display():
    if _OS == "Windows":
        try:
            ...
            print(f"[Settings] sleep_display falló: {e}")
    elif _OS == "Darwin":
        subprocess.run(["pmset", "displaysleepnow"], capture_output=True)
    else:
...
def dark_mode():
    if _OS == "Darwin":
        ...
        try:
            ...
            print(f"[Settings] dark_mode en registro falló: {e}")
    else:
        try:
            result = subprocess.run(
            ...
            print(f"[Settings] dark_mode en Linux falló: {e}")
...
def toggle_wifi():
    if _OS == "Darwin":
        ...
        try:
            ...
            print(f"[Settings] toggle_wifi en Windows falló: {e}")
    else:
        try:
            ...
            print(f"[Settings] toggle_wifi en Linux falló: {e}")
...
def restart_computer():
    if _OS == "Windows":
        ...
        subprocess.run(
            ["osascript", "-e", 'tell application "System Events" to restart'], capture_output=True
        )
    else:
...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        ...
        print(f"[Settings] La detección de intención falló: {e}")
...
def computer_settings(
    ...
    if not action:
        return "No se pudo determinar ninguna acción."
    ...
    if action in _DANGEROUS_ACTIONS:
        ...
        if confirmed not in ("yes", "true", "1", "confirm"):
            return (
                f"Esto va a {action} el ordenador. "
                f"Por favor, confirme llamando de nuevo con confirmed=yes."
            )

    if action == "volume_set":
        try:
            volume_set(int(value or 50))
            return f"Volumen ajustado a {value}%."
        except Exception as e:
    ...
    try:
        ...
        print(f"[Settings] La acción falló ({action}): {e}")
        return f"La acción falló ({action}): {e}"
