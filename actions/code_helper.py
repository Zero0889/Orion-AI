...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"orion_code{ext}"
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"orion_debug_{int(time.time())}.png"
...
def _edit_action(file_path, instruction, player) -> str:
    if not file_path:
        return "Por favor, proporcione una ruta de archivo para editar, señor."
    if not instruction:
        return "Por favor, describa qué cambio realizar, señor."
...
def _explain_action(file_path, code, player) -> str:
    if file_path and not code:
        ...
        if err:
            return err
    if not code:
        return "Por favor, proporcione código o una ruta de archivo para explicar, señor."
...
def _run_action(file_path, args, timeout, player) -> str:
    if not file_path:
        return "Por favor, proporcione una ruta de archivo para ejecutar, señor."
    p = Path(file_path)
    if not p.exists():
        return f"Archivo no encontrado: {file_path}"
    if player:
...
def _optimize_action(file_path, code, language, output_path, player) -> str:
    ...
    if file_path and not code:
        ...
        if err:
            return err
    if not code:
        return "Por favor, proporcione código o una ruta de archivo para optimizar, señor."
